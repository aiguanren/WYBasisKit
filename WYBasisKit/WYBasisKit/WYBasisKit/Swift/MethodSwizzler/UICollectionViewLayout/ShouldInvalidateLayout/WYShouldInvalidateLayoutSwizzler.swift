//
//  WYShouldInvalidateLayoutSwizzler.swift
//  WYBasisKit
//
...
/**
 ...
 - Parameters:
   - targetClass: 目标布局类。
   - intercept: 拦截决策闭包。返回 `.proceed` 则继续执行原始方法；返回 `.result(shouldInvalidate)` 则直接返回该布尔值，**不再调用原始方法**。参数依次为：布局对象、新边界。
   - before: 方法执行前回调。参数依次为：布局对象、新边界。无返回值。
   - after: 方法执行后回调。参数依次为：布局对象、新边界、原始返回值（`Bool`）。可返回一个新的 `Bool` 来替换原始返回值。
 */
internal func wy_swizzlerShouldInvalidateLayout(
    for targetClass: UICollectionViewLayout.Type,
    intercept: ((_ layout: UICollectionViewLayout, _ newBounds: CGRect) -> WYInterceptResult<Bool>)? = nil,
    ...
    // 生成存储 intercept 闭包的键，格式：wy_intercept_shouldInvalidateLayout_<类名>_<方法名>
    let interceptKey = wy_methodSwizzlerKey(for: .shouldInvalidateLayoutIntercept, className: className, selectorName: NSStringFromSelector(selector))
    
    // 加锁，保证对全局字典的修改线程安全
    WYHooksLock.lock()
    
    // 存储 intercept 闭包到全局字典
    WYShouldInvalidateLayoutInterceptMap[interceptKey] = intercept
    ...
    let newBlock: @convention(block) (UICollectionViewLayout, CGRect) -> Bool = { receiver, newBounds in
        ...
        var foundIntercept: ((UICollectionViewLayout, CGRect) -> WYInterceptResult<Bool>)? = nil
        while let cls = currentClass, cls != NSObject.self {
            let className = NSStringFromClass(cls)
            // 生成 intercept 闭包的键
            let key = wy_methodSwizzlerKey(for: .shouldInvalidateLayoutIntercept, className: className, selectorName: NSStringFromSelector(selector))
            WYHooksLock.lock()
            let intercept = WYShouldInvalidateLayoutInterceptMap[key]
            WYHooksLock.unlock()
            if intercept != nil {
        ...
        if let intercept = foundIntercept {
            switch intercept(receiver, newBounds) {
...
/// 独立存储 shouldInvalidateLayout 的 intercept 闭包
internal var WYShouldInvalidateLayoutInterceptMap: [String: (UICollectionViewLayout, CGRect) -> WYInterceptResult<Bool>] = [:]
