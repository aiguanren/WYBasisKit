...
public final class WYAudioKit: NSObject,
    ...
    private var pausedTasksInfo: [URL: DownloadTaskInfo] = [:] // 暂停的任务信息（原始URL -> 任务信息，包含resumeData）
    private var downloadGroups: [UUID: DownloadBatch] = [:]
    private var downloadProgresses: [URL: Double] = [:]       // 原始URL -> 当前进度（0-1）
    ...
    public func downloadRemoteAudio(remoteUrls: [URL],
        ...
        for originalURL in remoteUrls {
            ...
            if let pausedInfo = pausedTasksInfo[originalURL] {
                pausedTasksInfo.removeValue(forKey: originalURL)
            }
    ...
    public func pauseDownload(_ remoteUrls: [URL]?) {
        ...
        for originalURL in urls {
            // 如果任务已经在暂停字典中，直接忽略（避免重复暂停警告）
            if pausedTasksInfo[originalURL] != nil {
                print("ℹ️ [WYAudioKit] 任务已暂停，忽略: \(originalURL)")
                continue
            }
            
            guard let info = tasksInfo[originalURL], let task = info.task else {
                print("⚠️ [WYAudioKit] 无法暂停 \(originalURL)：任务不存在或已完成")
                continue
            }
            ...
            tasksInfo.removeValue(forKey: originalURL)
            // 保存当前进度
            let currentProgress = downloadProgresses[originalURL] ?? info.progress
            ...
            task.cancel { [weak self] resumeData in
                ...
                pausedInfo.task = nil
                pausedInfo.progress = currentProgress
                // 保存到暂停字典
                self.pausedTasksInfo[originalURL] = pausedInfo
                // 保持进度显示不变（不清零）
                self.downloadProgresses[originalURL] = currentProgress
    ...
    public func resumeDownload(_ remoteUrls: [URL]?) {
        var resumedUrls: [URL] = []
        let urls = remoteUrls ?? Array(pausedTasksInfo.keys)
        
        for originalURL in urls {
            guard var pausedInfo = pausedTasksInfo[originalURL],
                  let resumeData = pausedInfo.resumeData else {
                print("⚠️ [WYAudioKit] 无法恢复 \(originalURL)：缺少恢复数据")
                continue
            }
            
            let task = downloadSession.downloadTask(withResumeData: resumeData)
            task.resume()
            pausedInfo.task = task
            pausedInfo.resumeData = nil
            // 移回活跃任务字典
            tasksInfo[originalURL] = pausedInfo
            // 进度保持原有值
            downloadProgresses[originalURL] = pausedInfo.progress
            // 清理暂停字典
            pausedTasksInfo.removeValue(forKey: originalURL)
    ...
    public func cancelDownload(_ remoteUrls: [URL]?) {
        let urls = remoteUrls ?? Array(tasksInfo.keys) + Array(pausedTasksInfo.keys)
        let uniqueUrls = Set(urls)
        
        for originalURL in uniqueUrls {
            // 处理活跃任务
            if let info = tasksInfo[originalURL] {
                info.task?.cancel()
                tasksInfo.removeValue(forKey: originalURL)
            }
            // 处理暂停任务
            ...
            // 清理批次
            let batchID = tasksInfo[originalURL]?.batchID ?? pausedTasksInfo[originalURL]?.batchID
            if let batchID = batchID as UUID?,
    ...
    public func urlSession(_ session: URLSession,
        ...
        guard let newURL = request.url,
              let originalRemote = tasksInfo.first(where: { $0.value.task === task })?.key else {
    ...
    public func urlSession(_ session: URLSession,
        ...
        tasksInfo[originalURL] = info
        downloadProgresses[originalURL] = progress
    }
    ...
    public func urlSession(_ session: URLSession,
        ...
        do {
            ...
            downloadProgresses[originalURL] = 1.0
    ...
    private func stopDisplayLinkIfNeeded() {
        if !isRecording && !isPlaying && convertSessions.isEmpty && tasksInfo.isEmpty && pausedTasksInfo.isEmpty {
