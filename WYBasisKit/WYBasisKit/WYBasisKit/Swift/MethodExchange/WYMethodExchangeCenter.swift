...
public func wy_removeHooks(for anyClass: AnyClass, selector: Selector? = nil) {
    ...
    let shouldPopPrefix = "wy_shouldPop_\(String(describing: anyClass))_"
    WYHooksLock.lock()
    if let selector = selector {
        ...
        let shouldPopKey = shouldPopPrefix + NSStringFromSelector(selector)
...
public func wy_exchangePopViewController(
    ...
    let key = "wy_\(String(describing: navigationControllerClass))_\(NSStringFromSelector(selector))"
    typealias Args = Bool
    ...
    let shouldPopKey = "wy_shouldPop_\(String(describing: navigationControllerClass))_\(NSStringFromSelector(selector))"
    WYHooksLock.lock()
    ...
    let newBlock: @convention(block) (UINavigationController, Bool) -> UIViewController? = { receiver, animated in
        ...
        while let cls = currentClass, cls != NSObject.self {
            let key = "wy_shouldPop_\(String(describing: cls))_\(NSStringFromSelector(selector))"
            WYHooksLock.lock()
