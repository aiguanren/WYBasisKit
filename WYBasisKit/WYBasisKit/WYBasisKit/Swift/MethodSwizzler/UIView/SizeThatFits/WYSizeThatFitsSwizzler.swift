...
/**
 ...
 - Parameters:
   ...
   - intercept: 拦截决策闭包。返回 `.proceed` 则继续执行原始方法；返回 `.result(size)` 则直接返回该尺寸，**不再调用原始方法**。参数依次为：当前视图、建议的尺寸。
   - before: 方法执行前回调。参数依次为：当前视图、建议的尺寸。无返回值。
   - after: 方法执行后回调。参数依次为：当前视图、建议的尺寸、原始返回值（`CGSize`）。可返回一个新的 `CGSize` 来替换原始返回值。
 */
internal func wy_swizzlerSizeThatFits(
    for targetClass: UIView.Type,
    intercept: ((_ currentView: UIView, _ size: CGSize) -> WYInterceptResult<CGSize>)? = nil,
    ...
    // 生成存储 intercept 闭包的键，格式：wy_intercept_sizeThatFits_<类名>_<方法名>
    let interceptKey = wy_methodSwizzlerKey(for: .sizeThatFitsIntercept, className: className, selectorName: NSStringFromSelector(selector))
    
    // 加锁，保证对全局字典的修改线程安全
    WYHooksLock.lock()
    
    // 存储 intercept 闭包到全局字典
    WYSizeThatFitsInterceptMap[interceptKey] = intercept
    ...
    let newBlock: @convention(block) (UIView, CGSize) -> CGSize = { receiver, size in
        ...
        var foundIntercept: ((UIView, CGSize) -> WYInterceptResult<CGSize>)? = nil
        while let cls = currentClass, cls != NSObject.self {
            let className = NSStringFromClass(cls)
            // 生成 intercept 闭包的键
            let key = wy_methodSwizzlerKey(for: .sizeThatFitsIntercept, className: className, selectorName: NSStringFromSelector(selector))
            WYHooksLock.lock()
            let intercept = WYSizeThatFitsInterceptMap[key]
            WYHooksLock.unlock()
            if intercept != nil {
        ...
        if let intercept = foundIntercept {
            switch intercept(receiver, size) {
...
/**
 查找 sizeThatFits 的 intercept 闭包（沿继承链）
 ...
 */
internal func wy_findSizeThatFitsIntercept(for receiver: UIView, selector: Selector) -> ((UIView, CGSize) -> WYInterceptResult<CGSize>)? {
    let selName = NSStringFromSelector(selector)
    var currentClass: AnyClass? = type(of: receiver)
    while let cls = currentClass, cls != NSObject.self {
        let className = NSStringFromClass(cls)
        // 生成 intercept 闭包的键
        let key = wy_methodSwizzlerKey(for: .sizeThatFitsIntercept, className: className, selectorName: selName)
        WYHooksLock.lock()
        let intercept = WYSizeThatFitsInterceptMap[key]
        WYHooksLock.unlock()
...
/// 独立存储 sizeThatFits 的 intercept 闭包
internal var WYSizeThatFitsInterceptMap: [String: (UIView, CGSize) -> WYInterceptResult<CGSize>] = [:]
