//
//  WYIntrinsicContentSizeSwizzler.swift
//  WYBasisKit
//
...
/**
 ...
 - Parameters:
   ...
   - intercept: 拦截决策闭包。返回 `.proceed` 则继续执行原始方法；返回 `.result(size)` 则直接返回该尺寸，**不再调用原始方法**。参数为当前视图。
   - before: 方法执行前回调。参数为当前视图。无返回值。
   - after: 方法执行后回调。参数依次为：当前视图、原始返回值（`CGSize`）。可返回一个新的 `CGSize` 来替换原始返回值。
 */
internal func wy_swizzlerIntrinsicContentSize(
    for targetClass: UIView.Type,
    intercept: ((_ currentView: UIView) -> WYInterceptResult<CGSize>)? = nil,
    ...
    // 生成存储 intercept 闭包的键，格式：wy_intercept_intrinsicContentSize_<类名>_<方法名>
    let interceptKey = wy_methodSwizzlerKey(for: .intrinsicContentSizeIntercept, className: className, selectorName: NSStringFromSelector(selector))
    
    // 加锁，保证对全局字典的修改线程安全
    WYHooksLock.lock()
    
    // 存储 intercept 闭包到全局字典
    WYIntrinsicContentSizeInterceptMap[interceptKey] = intercept
    ...
    let newBlock: @convention(block) (UIView) -> CGSize = { receiver in
        ...
        var foundIntercept: ((UIView) -> WYInterceptResult<CGSize>)? = nil
        while let cls = currentClass, cls != NSObject.self {
            let className = NSStringFromClass(cls)
            // 生成 intercept 闭包的键
            let key = wy_methodSwizzlerKey(for: .intrinsicContentSizeIntercept, className: className, selectorName: NSStringFromSelector(selector))
            WYHooksLock.lock()
            let intercept = WYIntrinsicContentSizeInterceptMap[key]
            WYHooksLock.unlock()
            if intercept != nil {
...
/// 独立存储 intrinsicContentSize 的 intercept 闭包
internal var WYIntrinsicContentSizeInterceptMap: [String: (UIView) -> WYInterceptResult<CGSize>] = [:]
