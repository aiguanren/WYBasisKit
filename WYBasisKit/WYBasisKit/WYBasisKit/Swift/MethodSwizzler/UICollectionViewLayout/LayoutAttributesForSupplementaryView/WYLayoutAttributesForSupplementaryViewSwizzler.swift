//
//  WYLayoutAttributesForSupplementaryViewSwizzler.swift
//  WYBasisKit
//
...
/**
 ...
 - Parameters:
   - targetClass: 目标布局类。
   - intercept: 拦截决策闭包。返回 `.proceed` 则继续执行原始方法；返回 `.result(attributes)` 则直接返回该属性对象，**不再调用原始方法**。参数依次为：布局对象、元素类型、索引路径。
 ...
 */
internal func wy_swizzlerLayoutAttributesForSupplementaryView(
    for targetClass: UICollectionViewLayout.Type,
    intercept: ((_ layout: UICollectionViewLayout, _ elementKind: String, _ indexPath: IndexPath) -> WYInterceptResult<UICollectionViewLayoutAttributes?>)? = nil,
    ...
    // 生成存储 intercept 闭包的键，格式：wy_intercept_layoutAttributesForSupplementaryView_<类名>_<方法名>
    let interceptKey = wy_methodSwizzlerKey(for: .layoutAttributesForSupplementaryViewIntercept, className: className, selectorName: NSStringFromSelector(selector))
    
    // 加锁，保证对全局字典的修改线程安全
    WYHooksLock.lock()
    
    // 存储 intercept 闭包到全局字典
    WYLayoutAttributesForSupplementaryViewInterceptMap[interceptKey] = intercept
    ...
    let newBlock: @convention(block) (UICollectionViewLayout, String, IndexPath) -> UICollectionViewLayoutAttributes? = { receiver, elementKind, indexPath in
        ...
        var foundIntercept: ((UICollectionViewLayout, String, IndexPath) -> WYInterceptResult<UICollectionViewLayoutAttributes?>)? = nil
        while let cls = currentClass, cls != NSObject.self {
            let className = NSStringFromClass(cls)
            // 生成 intercept 闭包的键
            let key = wy_methodSwizzlerKey(for: .layoutAttributesForSupplementaryViewIntercept, className: className, selectorName: NSStringFromSelector(selector))
            WYHooksLock.lock()
            let intercept = WYLayoutAttributesForSupplementaryViewInterceptMap[key]
            WYHooksLock.unlock()
            if intercept != nil {
        ...
        if let intercept = foundIntercept {
            switch intercept(receiver, elementKind, indexPath) {
...
/// 独立存储 layoutAttributesForSupplementaryView 的 intercept 闭包
internal var WYLayoutAttributesForSupplementaryViewInterceptMap: [String: (UICollectionViewLayout, String, IndexPath) -> WYInterceptResult<UICollectionViewLayoutAttributes?>] = [:]
