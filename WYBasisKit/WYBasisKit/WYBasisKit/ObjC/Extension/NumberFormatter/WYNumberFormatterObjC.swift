...
@objc public extension NumberFormatter {
    ...
    @objc static func wy_roundToInteger(_ string: String?) -> String {
        return wy_roundToInteger(string: string)
    }
    
    /// 千位符之小数转百分数(1234.5678 -> 123,457%)
    @objc static func wy_decimalToPercent(_ string: String?, maximumFractionDigits: Int = 4) -> String {
        return wy_decimalToPercent(string: string, maximumFractionDigits: maximumFractionDigits)
    }
    
    /// 千位符之国际化格式小数(1234.5678 -> 1,234.5678)
    @objc static func wy_internationalizedFormat(_ string: String?, maximumFractionDigits: Int = 4) -> String {
