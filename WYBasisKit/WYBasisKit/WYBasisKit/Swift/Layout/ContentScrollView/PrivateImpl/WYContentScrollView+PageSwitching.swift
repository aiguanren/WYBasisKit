...
extension WYContentScrollView {
    ...
    func endInteractiveSlideDragIfNeeded() -> Bool {
        ...
        if canSwitchedPage {
            // 完成时系统减速会精确停在改写的惯性目标上(整页侧)，方向保持目标轴，提交交给调用方紧随的pauseScroll(下标经同下标语义落定)
            isInteractiveCrossAxisDrag = false
            isFinalizingSwitch = false
            return true
        }else {
            // 回弹时恢复原展示轴方向(拖动期间它已翻成目标轴方向，残留会让下一次拖动的展示轴推导/能力判定判反)与置顶层级，config复位让下次滑动能重新发willSwitch(没切成的语义=只发will然后弹回，下次滑动重新来一遍)
     ...
     *  @param direction       目标方向(同时决定.instant/.slide的落位偏移)
     *  @param preservesIndex  true为同下标跨轴(下标保持)，false为翻页跨轴(目标轴下标推进/回退)
     */
     ...
     *  @return entering=装好目标内容的预备页，hidden=动画结束后需要恢复alpha的同轴当前页
     */
