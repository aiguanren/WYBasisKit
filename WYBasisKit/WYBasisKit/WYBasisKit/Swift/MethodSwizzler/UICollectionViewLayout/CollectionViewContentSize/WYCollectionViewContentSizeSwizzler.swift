//
//  WYCollectionViewContentSizeSwizzler.swift
//  WYBasisKit
//
...
/**
 ...
 - Parameters:
   - targetClass: 目标布局类。
   - intercept: 拦截决策闭包。返回 `.proceed` 则继续执行原始方法；返回 `.result(size)` 则直接返回该尺寸，**不再调用原始方法**。参数为当前布局对象。
   - before: 方法执行前回调。参数为当前布局对象。无返回值。
   - after: 方法执行后回调。参数依次为：布局对象、原始返回值（`CGSize`）。可返回一个新的 `CGSize` 来替换原始返回值。
 */
internal func wy_swizzlerCollectionViewContentSize(
    for targetClass: UICollectionViewLayout.Type,
    intercept: ((_ layout: UICollectionViewLayout) -> WYInterceptResult<CGSize>)? = nil,
    ...
    // 生成存储 intercept 闭包的键，格式：wy_intercept_collectionViewContentSize_<类名>_<方法名>
    let interceptKey = wy_methodSwizzlerKey(for: .collectionViewContentSizeIntercept, className: className, selectorName: NSStringFromSelector(selector))
    
    // 加锁，保证对全局字典的修改线程安全
    WYHooksLock.lock()
    
    // 存储 intercept 闭包到全局字典
    WYCollectionViewContentSizeInterceptMap[interceptKey] = intercept
    ...
    let newBlock: @convention(block) (UICollectionViewLayout) -> CGSize = { receiver in
        ...
        var foundIntercept: ((UICollectionViewLayout) -> WYInterceptResult<CGSize>)? = nil
        while let cls = currentClass, cls != NSObject.self {
            let className = NSStringFromClass(cls)
            // 生成 intercept 闭包的键
            let key = wy_methodSwizzlerKey(for: .collectionViewContentSizeIntercept, className: className, selectorName: NSStringFromSelector(selector))
            WYHooksLock.lock()
            let intercept = WYCollectionViewContentSizeInterceptMap[key]
            WYHooksLock.unlock()
            if intercept != nil {
...
/// 独立存储 collectionViewContentSize 的 intercept 闭包
internal var WYCollectionViewContentSizeInterceptMap: [String: (UICollectionViewLayout) -> WYInterceptResult<CGSize>] = [:]
